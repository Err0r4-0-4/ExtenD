...
const Creators = React.memo(() => {
  ...
  useEffect( async () => {
    ...
    let res = await axios.post("http://localhost:3000/creator/creatorById", data, config);
    // console.log(res.data);
    setCreator(res.data.creator);

    res = await axios.post("http://localhost:3000/creator/getContracts",data, config);
    console.log(res);
    console.log("res")
  ...
  const onUploadHandler = (event) => {
    ...
    let config = {
      ...
      axios.post('http://localhost:3000/creator/uploadContract', formData, config)
        .then( response =>{
